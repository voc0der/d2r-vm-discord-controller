...
- The count is read off the party bar of a client that is already in the game, and only once every rostered bot has joined - mid join the bar lags the roster, and subtracting a roster that is still arriving would undercount the humans and fill the very slot the mode is holding.
