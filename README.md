...
Sleeping a host requires the account running D2RHost to hold the "Shut down the system" user right, because the sleep API needs `SE_SHUTDOWN_NAME` and—unlike shutdown and restart, which go through `shutdown.exe`—nothing enables it automatically. If a host cannot sleep, the command now says so in Discord rather than reporting the sleep as queued. A machine that accepts the call but does not actually suspend (Modern Standby laptops and Hyper-V hosts can do this) is also detected and reported, along with `powercfg /a` output naming which sleep states that machine really supports.
