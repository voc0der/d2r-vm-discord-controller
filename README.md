...
During follow-auto, five consecutive failures by one client to complete the outer desktop-to-lobby `menu_ready` warmup trigger a VM-safe restart of that client's physical node. A successful warmup (or a live preflight showing that the client is already menu-ready) resets its count; friend lookup, join, and other later check failures do not count. The node journals and stops only its configured VMs that were Running before it restarts. For a worker, follow-auto waits for a new worker connection generation and for that node's active expected VM accounts to return; for the master, it persists the follow options and whole active expected fleet in SQLite, then resumes the run after startup restores the journaled VMs. A failed restart queue is not retained as a pending resume and requires five new consecutive warmup failures before retrying.
