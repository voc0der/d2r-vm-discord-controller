...
Sleeping a host requires the account running D2RHost to hold the "Shut down the system" user right, because the sleep API needs `SE_SHUTDOWN_NAME` and—unlike shutdown and restart, which go through `shutdown.exe`—nothing enables it automatically. A worker older than `v0.2.217` does not contain that privilege fix, so the master rejects its sleep acknowledgement, keeps itself online, and gives the one-time update bootstrap instruction instead. Current builds also query Windows for S1/S2/S3 or Modern Standby support before acknowledging; a host with no usable sleep state returns `powercfg /a` diagnostics synchronously, so a worker failure keeps the master online. A call that Windows accepts but that returns without actually suspending is still detected and logged; the master also posts its own late failure to Discord.
