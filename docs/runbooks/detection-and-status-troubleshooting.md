...
When no D2R process is running, detailed status skips pixel classification and reports `NotRunning`. Desktop pixels are not evidence of a healthy game screen and must not clear a pending settings-repair journal. The semantic graphics-device-error dialog check still runs, because the launcher may own that dialog after D2R exits.
