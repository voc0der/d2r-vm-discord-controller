...
| Friend context Join Game | `FriendContextJoinGame` | `0.278,0.344` | `380,264` | Click/sample | `1366x768/lobby_right_click_friend_join_game_available.png` | Row-1 context-menu option; runtime clicks keep the same in-menu offset from the right-clicked row because the menu is anchored to the pointer position. |
...
| Current-character join restriction Cancel | `CannotJoinCurrentCharacterCancelButton` | `0.425,0.539` | `581,414` | Click/sample | `1366x768/cannot_join_game_with_current_character.png` | Dismisses the two-button join restriction without switching characters so follow-auto can report it and retry. |
...
| N (1-7) | `219 + (N-1)*72, 29` | left = `183 + (N-1)*72`, same y span |
