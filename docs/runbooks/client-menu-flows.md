...
The ready flow waits up to `d2rStartTimeoutSeconds` for D2R to appear, re-sending the launch command at `battleNetExecRetryDelaySeconds` and clicking Battle.net Play when the blue button is detected. After D2R exists, it best-effort focuses D2R, primes focus with a click at `ui.introSkipPoint`, default center, then sends real `G` presses every `ui.readyStartupSkipIntervalMs`, default 100 ms. Key presses use `keybd_event` plus virtual-key `SendInput`, and mouse clicks use `SetCursorPos` plus `mouse_event` so failures are visible in the VM desktop. It repeats the focus/click prime every tenth key burst. It stops only when character select is visually detected. The ready loop intentionally does not send Escape because an unrecognized character screen can interpret Escape as menu/exit input.
