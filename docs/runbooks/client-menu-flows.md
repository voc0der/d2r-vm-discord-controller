...
The ready flow waits up to `d2rStartTimeoutSeconds` for D2R to appear, re-sending the launch command at `battleNetExecRetryDelaySeconds` and clicking Battle.net Play when the blue button is detected. After D2R exists, it best-effort focuses D2R but does not block on focus verification or process-window geometry. It then runs a startup skip pump for up to `ui.readyStartupSkipSeconds`, default 25 seconds, sending safe continue input every `ui.readyStartupSkipIntervalMs`, default 250 ms, until character select is detected. After that startup phase, it samples the screen and sends the same continue input burst every pass. Unknown intro/video/loading states and the detected Diablo title splash, including the Connecting to Battle.net modal, get a configured center click, Space, Enter, then another center click. The ready loop intentionally does not send Escape because an unrecognized character screen can interpret Escape as menu/exit input. Clicks and keys are held briefly, and the agent also posts the same ready burst directly to the D2R window handle as a fallback. Each normal nudge waits a randomized `readyNudge` interval before the next sample. The agent only marks the client ready after Play and Lobby are visually detected, or fails after `ui.characterScreenReadyTimeoutSeconds` with the last detected ready state and ready input burst count.
