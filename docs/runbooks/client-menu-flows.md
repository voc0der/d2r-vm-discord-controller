...
At the start of a pending account's follow-auto check, an in-game safety match now enters an explicit stale-game recovery instead of repeatedly suppressing the Lobby click. The agent sends the safe `G` legacy-graphics toggle first, waits for the HUD to redraw, and takes a fresh strict modern/legacy globe sample. A confirmed game is left with the normal retrying Save and Exit flow, then the account rejoins on the next follow-auto cycle. A still-inconclusive sample performs no menu or exit click and reports that it is waiting; a fresh not-in-game result resumes ordinary Lobby navigation. This also covers a host or agent restart that loses the in-memory joined set while the D2R client remains in the previous game.

The all-joined watcher also detects the inverse bookkeeping failure: one account remains marked joined but is actually alone in a different game. One leader miss contradicted by another VM remains transient. Two independently-contradicted misses from the same account trigger a targeted resync: only that client receives Save and Exit, its joined flag is removed, and the normal pending-account flow rejoins it without advancing the game number or disturbing the healthy clients. Each account gets at most one targeted resync per game, preventing a persistent per-VM fingerprint mismatch from creating a leave/rejoin loop.
