...
| Dismiss current-character join restriction | `CannotJoinCurrentCharacterCancelButton` | `581,414` | `1366x768/cannot_join_game_with_current_character.png` |
...
`/d2r join` with no arguments, or with `auto:true` (issue #20, item 7), needs a template set first and assumes a human - not one of this bot's own VMs - creates each numbered game externally using that same naming (the `delay` flag, default 0, is the wait before each join attempt specifically so a sorceress can teleport to a boss alone before the bots pile in and spike the difficulty). Each cycle: mint the next number, try to join all online accounts; once everyone's in, request fresh player-count samples until the count drops below where it was right after joining; then leave-all and advance to the next number. The player-count-drop poll is dynamic from the auto session pace: unknown/first game and 10-minute average games use 15s, 5-minute average games use 12s, and 1-minute average games use 6s. Runs until `/d2r join auto:false`, an idle timeout, or the template being cleared - all of it posted to the invoking channel directly rather than as interaction follow-ups, since a multi-cycle farming run easily outlives a follow-up token's ~15 minute lifetime. Deliberately does not touch the create/join session message (`_activeSessionMessage`) - an unrelated manual run of either could be using it concurrently.
...
![Current character cannot join 1366x768](assets/d2r-ui/1366x768/cannot_join_game_with_current_character.png)
...
`/d2r follow bind:true [friend-row:<n>]` captures a small grid-sample "fingerprint" of the selected friend row's name-text area (`FriendFingerprint` in `AgentCommon`, captured via `WindowsInput.CaptureFingerprintGrid` - the same BitBlt-into-a-local-bitmap approach as every other classifier in this file, just a wide-short grid over the name line instead of a square grid over a threshold region) instead of a literal screenshot crop - smaller to transport over the existing JSON command protocol, no new image-codec dependency, and tolerant of normal rendering noise the same way every other pixel classifier here is. If `friend-row` is omitted, bind uses the VM's configured default friend row, normally row 1. The operator is responsible for the target friend actually being in the selected row at bind time; the command has no way to identify who it captured, only where it looked.
...
The game watcher also detects the inverse bookkeeping failure: one account remains marked joined but is actually alone in a different game. One leader miss contradicted by another VM remains transient. Two independently-contradicted misses from the same account trigger a targeted resync: that exact account is moved from joined to recovery-pending before Save and Exit is dispatched, so a timeout, ambiguous reply, or disconnect cannot leave stale joined bookkeeping behind. A joined account that drops offline during the active watcher is moved to the same recovery state without sending an unavailable leave command; it cannot make the remaining online subset look all-joined, and after reconnecting it must complete the normal menu recovery before its recovery flag clears. The healthy clients keep watching the current game during that recovery and follow the leader when it leaves; the isolated recovery itself does not advance the game number. Each account gets at most one targeted resync per game, preventing a persistent per-VM fingerprint mismatch from creating a leave/rejoin loop.
...
If joining surfaces `You cannot join the game with your current character`—for example because of a difficulty, level, capacity, or other game restriction—follow-auto detects the modal's distinct Cancel/Switch Characters button pair, clicks Cancel, and updates its persistent monitor with that exact restriction. It remains active and retries the bound friend's Join Game action on the normal cadence. Once the user creates a compatible replacement game, the same flow joins it without needing a restart or another command. A leftover modal from a cancelled or timed-out attempt is recognized and dismissed by `menu_ready` before it sends any generic startup input; the ready detector also treats the modal as a terminal state so a late-arriving overlay cannot trap the startup loop ahead of `menu_follow_auto_check`.

If joining surfaces `Game is full` (`assets/d2r-ui/1366x768/game_is_full.png`), the agent distinguishes it from the other messages that share the identical generic OK box by its single short text line (`IsGameFullDialogOpen`, see the pixel-classifier catalog), clicks OK, stays warm at the lobby, and reports `joinBlockReason=gameIsFull` instead of entering the generic dismiss-and-reselect retry. The host allows **4 total attempts against one game** (the initial detection plus 3 retries on the normal follow cadence, counted by `FollowAutoAccountState.RecordGameFullStrike`); if the game still reads full, that account is **parked**: it stays out of the pending scan, the monitor names it, and the game watcher continues with the accounts that did get in (parked accounts are excluded from the watchable set and from the pulse/confirm vantage rotation, since a lobby client can neither see the party bar nor report a fresh player count). This is deliberate slot etiquette, not a failure mode: with 2 humans + 6 VMs filling an 8-player game, a human who drops mid-game must be able to reclaim their own slot - a parked bot never takes it. When the bound leader leaves and the fleet advances, `ClearGameFullParking` un-parks everyone with a fresh retry budget and they rejoin the next game with the normal scan. Two edge behaviors: an account pulled into recovery keeps its strike count, so a still-full game re-parks it after one attempt rather than granting a fresh budget; and if *every* online account is parked (no bot ever got in), there is no vantage to see that game end - the monitor says so explicitly and the run waits (idle timeout still applies), so restart follow-auto (Stop, then Follow) if the leader has already moved on. Manual `join`/`join-all` flows are unchanged: outside follow-auto, a full-game dialog still follows the generic error-dialog dismiss-and-retry path.

Follow checks, menu warmup, recovery, and Save and Exit are scheduled per account. The fleet loop consumes completed results without waiting for slower accounts, and monitors joined clients even while others are pending. Late check results from an abandoned game are discarded; the account remains reserved until the old command finishes and then gets a fresh check. A worker restart releases its accounts once a new worker connection is confirmed, so returning VM agents can follow without waiting for their siblings. Stop cancels and drains outstanding work before the run releases ownership.
...
- Error states: realm/server issue (game full is now captured: `1366x768/game_is_full.png`).
