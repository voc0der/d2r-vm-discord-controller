...
The ready flow waits up to `d2rStartTimeoutSeconds` for D2R to appear, re-sending the launch command at `battleNetExecRetryDelaySeconds` and clicking Battle.net Play when the blue button is detected. After D2R exists, it best-effort focuses D2R but does not block on focus verification or process-window geometry. It then presses `G` every `ui.readyStartupSkipIntervalMs`, default 100 ms, and posts the same `G` key directly to the D2R window until character select is detected or `ui.readyStartupSkipSeconds`, default 120 seconds, expires. The ready loop intentionally does not send Escape because an unrecognized character screen can interpret Escape as menu/exit input. The agent only marks the client ready after Play and Lobby are visually detected, or fails after the skip loop timeout with the last detected ready state and ready input count.
