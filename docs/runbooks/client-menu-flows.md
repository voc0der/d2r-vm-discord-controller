...
Bind/auto/follow first make the friend list idempotently visible: if the drawer is closed they single-click `LobbyPartyIcon`; if it is open they leave it alone; if the Friends accordion is collapsed they single-click `FriendsAccordionHeader`. These two controls use `ClickD2RStatefulToggle` instead of the normal redundant `ClickD2R` click stack because a second delivered click reverses the toggle state. This avoids the old blind party-icon click that closed an already-open drawer, the redundant-input double-toggle that opened and immediately closed the drawer, and capturing the empty black drawer body after a fresh drawer open.
