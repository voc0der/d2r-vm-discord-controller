...
| `IsGameFullDialogOpen` (`VmOperations`) + `D2RScreenClassifier.IsGameFullDialogTextBand` | generic-dialog gate above first, then text center `0.500,0.457` `0.055x0.034`; left flank `0.435,0.457` `0.055x0.034`; right flank `0.565,0.457` `0.055x0.034`; second line `0.500,0.478` `0.20x0.026` - all four at the denser `GameFullTextBandSampleGrid = 17` (a 9-grid lands between the thin glyphs: center std drops 45.7 -> 23.0) | "Game is full" renders in the *identical* generic OK box; only the message text differs, and it is the sole variant whose single short line leaves both text-row flanks **and** the second-line band empty. Center must read text (`LuminanceStdDev > 20 && BrightRatio > 0.02`); flanks/second line must be empty body (`Samples > 0 && LuminanceStdDev < 15 && BrightRatio < 0.02 && DarkRatio > 0.95`). Measured: `game_is_full.png` flanks std=3.3/bright=0.000/dark=1.00 vs every other generic-dialog capture with at least one flank std >= 28.4 and bright >= 0.042. Checked BEFORE the generic `ErrorDialog` classification in `WaitForGameEntryAsync` and before the friend-flow's dismiss-and-reselect retry: a full game must never be retry-hammered, or the bot snipes the first freed (human) slot. Recovery clicks OK, returns `GameEntryWaitResult.GameIsFull`, and follow-auto reports `joinBlockReason=gameIsFull` for the host's 4-attempt park counter (see `FollowAutoAccountState.MaxGameFullAttempts`). |
