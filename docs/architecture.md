...
A satellite that starts an update must exit. The updater script blocks on `Wait-Process` for the process that launched it before it replaces a single file, so a satellite that reports "update started" and keeps running is never updated at all - the script waits forever and the exe stays locked. The VM agent has always answered `self_update` with `ExitAfterResult`; worker builds v0.2.213-v0.2.219 did not, which is why a node could report "update started" on release after release and stay pinned to one build while every VM around it moved on. Those builds need one local PC restart, after its VMs are stopped, so the normal startup updater can apply the current release; the master cannot safely send that remote restart because these builds do not advertise the VM-safe host-power capability. The master's update notification says so explicitly. Starting with v0.2.220, the worker returns `ExitAfterResult` and also calls `StopApplication`, because unlike the VM agent - where the agent link *is* the process - a worker is a web host and the link loop ending alone leaves it running.
...
Discord `/d2r system sleep` defaults to every node that is online when the command is handled, matching the post-follow quick `Sleep` action: parking for the night should not leave a worker awake burning power and running VMs. `/d2r system shutdown|restart` still default to the master node, because those are normally aimed at one machine. `node:<node-id>` scopes the action to one known node, including the master; `all:false` narrows sleep back to the master alone; `all:true` widens shutdown/restart to the master and every online worker. Fleet-wide actions are prepared on workers first and the master last so the master can forward every worker command before powering itself down. Offline workers are listed and skipped; if a selected worker fails to confirm the action, the master remains online for recovery.

Every physical node owns its VM safety transaction locally through `VmPowerLifecycleCoordinator`; the master does not try to recover a worker's VMs across the network. The coordinator derives its target set from the distinct, non-empty `AccountConfig.VmName` values owned by that node, still applying the node's `allowedVmNamePrefixes`. It first reads every target's Hyper-V state. `Running` is eligible to stop, `Off` is deliberately untouched, and an unreadable, transitional, paused, saved, or otherwise unknown state fails the preparation without forcing it. This applies to sleep, the Modern Standby hibernation fallback, shutdown, and restart.

The eligible `Running` set is committed to the local SQLite `host_vm_resume` table before the first `Stop-VM`. Each stop must then be followed by an `Off` state. That write-before-stop ordering is crash-safe: if D2RHost exits halfway through the pass, the next process still knows the complete set that was running beforehand. A VM that independently becomes `Off` between the two state checks is removed from the journal because D2RHost did not stop it. If any stop or confirmation fails, the physical power action is not queued and the coordinator immediately runs the same idempotent restore path over the journal; entries that cannot be rolled back remain durable.

A successful preparation also arms one in-memory host-transition lease. While it is armed, a second physical-host power command fails before it can consume or replace the first command's live journal. Resume, exception recovery, and the shutdown/restart watchdog release the lease through the restore path. The lease is armed and released even when the eligible `Running` set is empty, because overlapping physical transitions are still unsafe without journal entries.

Restore starts only journaled VMs. An entry already `Running` is cleared without issuing `Start-VM`; an `Off` entry is started and cleared only after it confirms `Running`; any other state or command failure remains pending. `SetSuspendState` blocks the D2RHost process until an ordinary sleep resumes, so `HostSystemOperations` runs restore immediately after that call returns and retries pending work every 15 seconds. It uses the same retry loop after aborted preparation or a power-action exception. Shutdown and restart additionally arm a 30-second in-process watchdog: if `shutdown.exe` returned but D2RHost was not terminated and the journal still has entries, the action is considered failed and those VMs are restored. A real shutdown/restart, or a hibernation path that restarts the process, is recovered during Program startup before normal Discord/worker operation; `VmPowerRestoreService` retries startup-time failures while entries remain. Each worker uses its own configured `databasePath`, so recovery is independent of master availability and machine boot order.

Sleep resolves to a mechanism per machine. A host with any legacy standby state (S1/S2/S3) is suspended with `SetSuspendState`. A host with none - a Modern Standby laptop, where firmware supports no legacy standby at all - is hibernated with `shutdown /h` instead, and the command says so in its reply rather than promising "sleep". Modern Standby is deliberately not treated as reachable: entering S0 low-power idle means turning off the display in the interactive session, which a session-0 service cannot do, and `SetSuspendState` returns ERROR_NOT_SUPPORTED (50) on such a machine from a SYSTEM service and an elevated interactive session alike. A machine with no legacy standby *and* hibernation disabled has no reachable state, and the command fails with the one command that fixes it: `powercfg /hibernate on`.

Legacy S1/S2/S3 standby has no `shutdown.exe` equivalent, so that path calls `SetSuspendState` directly. The API requires `SE_SHUTDOWN_NAME` to be *enabled* in the process token - running elevated is not enough, because an administrator's token holds the privilege in the disabled state and the call then fails with ERROR_PRIVILEGE_NOT_HELD. `TryPrepareSleep` queries `GetPwrCapabilities`; `WindowsShutdownPrivilege` enables the privilege for the direct standby path, while the S4 fallback uses `shutdown.exe /h`. These checks happen synchronously before the command answers, and a capability failure includes `powercfg /a` diagnostics, so an incapable worker returns a failed command result and keeps the master online.

Worker rollout is capability-gated rather than version-gated. `WorkerNodeOperations` includes `vmSafeHostPowerTransitions:true` in every status heartbeat. `AgentRegistry` resets `StatusReceivedAt` when a worker socket reconnects while retaining its last inventory for display, and `FleetRegistry` accepts the capability only when the worker is connected and a status frame has been received on that exact connection. Missing, false, malformed, cached-from-an-older-connection, or wrong-node inventory all fail closed. `FleetHostOperations` applies that check before dispatching `system_sleep`, `system_shutdown`, or `system_restart`, then binds the send to the connection generation that supplied the status; a reconnect between authorization and send is rejected and must advertise again. Thus an old or just-reconnected worker cannot acknowledge a legacy physical-host-only implementation of the same command name.

A worker cannot post to Discord, so anything it discovers after a command has already answered rides its heartbeat instead: `WorkerNodeStatus.Alerts` is a small bounded mailbox the worker fills and clears as it sends, and the master drains into its notification queue. A failed sleep is the motivating case - there is no command result left to fail, and the worker's log sits on the machine nobody is looking at.

Sleep diagnostics come from PowrProf directly - `GetPwrCapabilities` for the S1/S2/S3/S4/Modern-Standby flags, `PowerGetActiveScheme` for the active scheme - before `powercfg` is consulted. Stripped Windows builds are exactly the machines where sleep breaks *and* where `powercfg.exe` and the Settings power pages are missing or broken, so a diagnostic that shells out reports nothing precisely when it is needed. An unreadable active power scheme is called out on its own: it breaks the Settings power pages and a programmatic sleep in the same way, so both symptoms together point at the scheme rather than at this app.

`SetSuspendState` blocks until the machine resumes, so a call that returns immediately means the transition was refused rather than slept-and-woken. A return faster than five seconds is therefore treated as a late failure and logged with `powercfg /a` output naming which sleep states that machine reports; the master also posts its own late failure to Discord, while a worker retains it in its local log. This matters because the symptom is invisible: on a headless host with the screen already off, a sleep that never happened looks exactly like one that worked. The post-follow quick `Sleep` action captures one online-node target set, reconciles clients that appear on those nodes during its quit pass, then uses the same worker-first behavior.
