...
A satellite that starts an update must exit. The updater script blocks on `Wait-Process` for the process that launched it before it replaces a single file, so a satellite that reports "update started" and keeps running is never updated at all - the script waits forever and the exe stays locked. The VM agent has always answered `self_update` with `ExitAfterResult`; the worker D2RHost did not, which is why a node could report "update started" on release after release and stay pinned to one build while every VM around it moved on. A worker also needs `StopApplication`, because unlike the VM agent - where the agent link *is* the process - a worker is a web host and the link loop ending leaves it running.
