...
Sleep is the only power action without a `shutdown.exe` equivalent, so it calls `SetSuspendState` directly. That API requires `SE_SHUTDOWN_NAME` to be *enabled* in the process token - running elevated is not enough, because an administrator's token holds the privilege in the disabled state and the call then fails with ERROR_PRIVILEGE_NOT_HELD. `TryPrepareSleep` queries `GetPwrCapabilities` for S1/S2/S3 or Modern Standby support, then `WindowsShutdownPrivilege` enables the privilege. Both checks happen synchronously before the command answers, and a capability failure includes `powercfg /a` diagnostics, so an incapable worker returns a failed command result and keeps the master online. A remote worker reporting a build older than v0.2.217 (or no parseable build) is rejected before dispatch because it predates this preparation path; that result explains the one-time update bootstrap.
